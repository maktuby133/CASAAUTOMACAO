...
// 🔥 CORREÇÃO DE SEGURANÇA: Validar variáveis de ambiente críticas
const REQUIRED_ENV_VARS = ['VAPID_PUBLIC_KEY', 'VAPID_PRIVATE_KEY', 'SESSION_SECRET'];
REQUIRED_ENV_VARS.forEach(envVar => {
  if (!process.env[envVar] && envVar.includes('VAPID')) {
    console.warn(`⚠️  AVISO: ${envVar} não configurada. Usando valores de desenvolvimento.`);
  }
});
...
// 🔥 CORREÇÃO DE SEGURANÇA: Middlewares de segurança
app.use(helmet({
...
const allowedOrigins = process.env.ALLOWED_ORIGINS ? process.env.ALLOWED_ORIGINS.split(',') : [
  'http://localhost:3000',
  'https://casaautomacao.onrender.com'
];
...
// 🔥 CORREÇÃO: Garantir que diretório de backups existe
...
// 🔥 NOVO: Sistema de lock para evitar race conditions
class StateManager {
  constructor() {
    this.lock = false;
    this.queue = [];
...
// 🔥 CORREÇÃO: Função de backup automático
function createBackup(filePath, data) {
  try {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const backupFile = path.join(BACKUP_DIR, `${path.basename(filePath, '.json')}_backup_${timestamp}.json`);
    fs.writeFileSync(backupFile, JSON.stringify(data, null, 2));
    console.log(`💾 Backup criado: ${backupFile}`);
    
    // Manter apenas últimos 10 backups
    const files = fs.readdirSync(BACKUP_DIR)
      .filter(f => f.includes(path.basename(filePath, '.json')))
      .sort()
      .reverse();
...
// 🔥 CORREÇÃO: Carregar subscriptions salvas com validação
    ...
    try {
      if (fs.existsSync(SUBSCRIPTIONS_FILE)) {
        ...
        const validSubscriptions = subscriptions.filter(sub => 
          sub && sub.endpoint && typeof sub.endpoint === 'string' && sub.endpoint.length > 0
        );
        
        if (validSubscriptions.length !== subscriptions.length) {
          console.warn(`⚠️  Removidas ${subscriptions.length - validSubscriptions.length} subscriptions inválidas`);
        }
        
        pushSubscriptions = validSubscriptions;
        console.log(`📱 ${validSubscriptions.length} subscriptions carregadas do arquivo`);
      }
    } catch (error) {
...
// 🔥 CORREÇÃO: Salvar subscriptions com backup
function saveSubscriptions() {
  return stateManager.withLock(() => {
    try {
      createBackup(SUBSCRIPTIONS_FILE, pushSubscriptions);
...
// 🔥 CORREÇÃO: Carregar estado com validação e recuperação
function loadState() {
  return stateManager.withLock(() => {
    try {
      if (fs.existsSync(STATE_FILE)) {
        ...
        // Validar e corrigir estrutura do estado
...
// 🔥 NOVO: Validar e corrigir estrutura do estado
function validateAndFixState(state) {
  const defaultState = createInitialState();
  
  // Garantir que todas as propriedades necessárias existem
  const fixedState = { ...defaultState, ...state };
  
  // Validar lâmpadas
  if (!fixedState.lights || typeof fixedState.lights !== 'object') {
    fixedState.lights = { ...defaultState.lights };
  } else {
    fixedState.lights = { ...defaultState.lights, ...fixedState.lights };
  }
  
  // Validar tomadas
  if (!fixedState.outlets || typeof fixedState.outlets !== 'object') {
    fixedState.outlets = { ...defaultState.outlets };
  } else {
    fixedState.outlets = { ...defaultState.outlets, ...fixedState.outlets };
  }
  
  // Validar irrigação
  if (!fixedState.irrigation || typeof fixedState.irrigation !== 'object') {
    fixedState.irrigation = { ...defaultState.irrigation };
  } else {
    fixedState.irrigation = { ...defaultState.irrigation, ...fixedState.irrigation };
  }
  
  // Validar dados dos sensores
  if (!Array.isArray(fixedState.sensorData)) {
    fixedState.sensorData = [];
  }
  
  // Limitar histórico de sensores
  if (fixedState.sensorData.length > 200) {
    fixedState.sensorData = fixedState.sensorData.slice(0, 200);
  }
  
  return fixedState;
}

// 🔥 NOVO: Tentar carregar do backup
function tryLoadFromBackup() {
  try {
    const files = fs.readdirSync(BACKUP_DIR)
      .filter(f => f.includes('devices-state_backup'))
      .sort()
      .reverse();
    
    if (files.length > 0) {
      const backupFile = path.join(BACKUP_DIR, files[0]);
      console.log(`🔄 Tentando carregar backup: ${backupFile}`);
...
// 🔥 CORREÇÃO: Salvar estado com backup e validação
function saveState(state) {
  return stateManager.withLock(() => {
    try {
      const validatedState = validateAndFixState(state);
      createBackup(STATE_FILE, validatedState);
      fs.writeFileSync(STATE_FILE, JSON.stringify(validatedState, null, 2));
...
// 🔥 CORREÇÃO: Atualizar status do ESP32 com validação
function updateESP32Status(device, ip) {
  const validDeviceId = device && typeof device === 'string' ? device.substring(0, 50) : 'ESP32-CASA-AUTOMACAO-V3';
  const validIp = ip && typeof ip === 'string' ? ip : 'Desconhecido';
...
// 🔥 CORREÇÃO: Verificar se ESP32 está conectado com timeout configurável
function checkESP32Connection(timeoutMinutes = 2) {
  if (esp32Status.lastHeartbeat) {
    ...
    if (timeSinceLastHeartbeat > timeoutMs) {
      ...
      console.log(`🔴 ESP32 desconectado - último heartbeat: ${Math.round(timeSinceLastHeartbeat / 1000)}s atrás`);
    }
  }
...
// 🔥 CORREÇÃO: Gerenciar subscriptions existentes com validação
function managePushSubscription(newSubscription) {
  ...
  if (typeof newSubscription.endpoint !== 'string' || newSubscription.endpoint.length === 0) {
    return { error: 'Endpoint da subscription inválido' };
  }
...
// 🔥 CORREÇÃO: Função para verificar e enviar alertas de gás com cooldown
function checkGasAlerts(gasLevel) {
  // Validar nível de gás
  if (typeof gasLevel !== 'number' || isNaN(gasLevel) || gasLevel < 0) {
    console.warn('⚠️  Nível de gás inválido recebido:', gasLevel);
    return;
  }
  ...
  // Cooldown para evitar notificações excessivas
  const cooldownCritical = 30000; // 30 segundos
  const cooldownWarning = 60000;  // 1 minuto

  // Alerta crítico (acima de 500)
  if (gasLevel > 500 && !activeGasAlerts.critical && timeSinceLastAlert > cooldownCritical) {
    sendGasAlert(gasLevel, 'critical');
  }
  // Alerta de aviso (acima de 300)
  else if (gasLevel > 300 && !activeGasAlerts.warning && timeSinceLastAlert > cooldownWarning) {
...
// 🔥 CORREÇÃO: Função para buscar dados do clima com cache
let weatherCache = {
  data: null,
  timestamp: null,
...
async function fetchWeatherData() {
  ...
  if (weatherCache.data && weatherCache.timestamp && 
    ...
    console.log('🌤️ Retornando dados do clima do cache');
...
function converterProgramacoesParaESP32(programacoesFrontend) {
  ...
  programacoesLimitadas.forEach((prog, index) => {
    // Validar programação
    if (!prog.hora || !prog.dias || !Array.isArray(prog.dias)) {
      console.warn(`⚠️  Programação ${index + 1} inválida, ignorando`);
      return;
    }
...
// 🔥 CORREÇÃO: Inicialização assíncrona correta
async function initializeData() {
  try {
    devicesState = await loadState();
    await loadSubscriptions();
    console.log('✅ Dados inicializados com sucesso');
  } catch (error) {
    console.error('❌ Erro na inicialização dos dados:', error);
    devicesState = createInitialState();
    pushSubscriptions = [];
  }
}
...
// 🔥 CORREÇÃO DE SEGURANÇA: Middleware de autenticação melhorado
...
// 🔥 CORREÇÃO DE SEGURANÇA: Login com rate limiting e validação
app.post('/api/login', loginLimiter, (req, res) => {
  ...
  const cleanPassword = password.trim().substring(0, 100);
  
  console.log('🔐 Tentativa de login:', { username: cleanUsername });
  
  // 🔥 CORREÇÃO CRÍTICA: Não usar senha hardcoded em produção
  const validUsername = process.env.ADMIN_USERNAME || 'admin';
  const validPassword = process.env.ADMIN_PASSWORD || 'admin123';
  
  if (cleanUsername === validUsername && cleanPassword === validPassword) {
    // 🔥 CORREÇÃO: Cookie seguro
    res.cookie('authToken', 'admin123', {
      maxAge: 24 * 60 * 60 * 1000, // 24 horas
      httpOnly: true, // 🔥 IMPORTANTE: Prevenir XSS
...
app.get('/api/esp32-status', (req, res) => {
  res.json({
    ...
    uptime: esp32Status.lastHeartbeat ? Date.now() - esp32Status.lastHeartbeat : null
  });
});
...
app.get('/api/sensor-data', (req, res) => {
  ...
  const sensorData = (devicesState.sensorData || []).map(data => {
    ...
    return {
      ...data,
      humidity: Math.round(humidity * 10) / 10, // 1 casa decimal
  ...
  res.json({ 
    data: sensorData.slice(0, 50), // Limitar resposta
    esp32: { connected: espConnected },
    summary: {
...
// Rota para desativar notificações push
...
// 🔥 CORREÇÃO: ESP32 envia dados com validação
app.post('/api/data', async (req, res) => {
  ...
  if (typeof temperature === 'undefined' && typeof humidity === 'undefined' && typeof gas_level === 'undefined') {
    return res.status(400).json({ error: 'Dados de sensores inválidos' });
  }
  ...
  let processedHumidity = parseFloat(humidity);
  if (isNaN(processedHumidity)) processedHumidity = 0;

  let processedTemperature = parseFloat(temperature);
  if (isNaN(processedTemperature)) processedTemperature = 0;

  let processedGasLevel = parseFloat(gas_level);
  if (isNaN(processedGasLevel)) processedGasLevel = 0;

  const newData = {
    temperature: Math.round(processedTemperature * 10) / 10,
    humidity: Math.round(processedHumidity * 10) / 10,
    gas_level: Math.round(processedGasLevel),
    gas_alert: !!gas_alert || processedGasLevel > 300,
    device: (device && typeof device === 'string') ? device.substring(0, 50) : 'ESP32',
    heartbeat: !!heartbeat,
    wifi_rssi: parseInt(wifi_rssi) || 0,
  ...
  await stateManager.withLock(async () => {
    ...
    if (devicesState.sensorData.length > 200) {
      devicesState.sensorData = devicesState.sensorData.slice(0, 200);
    }
...
app.post('/api/confirm', async (req, res) => {
  ...
  await stateManager.withLock(async () => {
    ...
    if (req.body.irrigation) {
      ...
      const espBombaEstado = !!req.body.irrigation.bomba_irrigacao;
      const espModoAuto = !!req.body.irrigation.modo_automatico;
...
app.post('/api/irrigation/save', async (req, res) => {
  try {
    ...
    if (!modo || !['manual', 'automatico'].includes(modo)) {
      return res.status(400).json({ error: 'Modo de irrigação inválido' });
    }
...
// 🔥 NOVO: Rota para backup manual
app.post('/api/backup', (req, res) => {
  try {
    createBackup(STATE_FILE, devicesState);
    createBackup(SUBSCRIPTIONS_FILE, pushSubscriptions);
...
app.listen(PORT, () => {
  console.log(`\n🔥 Servidor Automação V3.0 SEGURO rodando na porta ${PORT}`);
  console.log(`🌐 Acesse: http://localhost:${PORT}`);
  console.log(`📡 Monitoramento ESP32: ATIVADO`);
  console.log(`💧 Sistema de Irrigação: OTIMIZADO`);
  console.log(`🔔 Sistema de Notificações Push: SEGURO`);
  console.log(`🚨 Alertas de Gás: COM COOLDOWN`);
  console.log(`🔐 Sistema de Login: PROTEGIDO`);
  console.log(`📊 Sensores: COM VALIDAÇÃO`);
  console.log(`📱 Notificações Push: ${pushSubscriptions.length} dispositivos registrados`);
  console.log(`🔒 Segurança: HELMET + RATE LIMITING + VALIDAÇÃO`);
  console.log(`💾 Backup: AUTOMÁTICO (últimos 10 backups)\n`);
  
  // Log de configurações de segurança
  if (process.env.NODE_ENV === 'production') {
    console.log('🚀 MODO PRODUÇÃO: Segurança máxima ativada');
  } else {
    console.log('🔧 MODO DESENVOLVIMENTO: Usando configurações de desenvolvimento');
  }
});
